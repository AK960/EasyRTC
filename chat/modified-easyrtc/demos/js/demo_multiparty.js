...
function sendMessage() {
    ...
    if (text.trim().length === 0) { // Don't send just whitespace
        return;
    }
...
function connect() {
    ...
    easyrtc.connect("easyrtc.multiparty", loginSuccess, loginFailure);
}
...
function convertListToButtons(roomName, occupants, isPrimary) {
    ...
    // Iterate through the occupants and create buttons
    for (var easyrtcid in occupants) {
        var button = document.createElement('button');
        
        // Set the button's onclick function to send a message to the selected occupant
    ...
    // If no occupants are present, display a message
...
function sendStuffWS(easyrtcid) {
    var text = document.getElementById('sendMessageText').value;
...
function addToConversation(who, msgType, content) {
    ...
    messageElement.innerHTML = "<b>" + who + ":</b>&nbsp;" + content;

    // Append the message element to the conversation div
